...
def test_user_login_success(user, monkeypatch):
    """Teste la connexion avec des identifiants valides."""
    # Saisie simulée : username, puis password
    # La chaîne doit inclure des retours à la ligne ('\n') pour simuler la touche Entrée.
    input_data = 'test_admin\ntest_password\n'
    ...
    result = runner.invoke(
        ...
        ['user-login'],
        input=input_data,
        # catch_exceptions=False est utile pour afficher la trace complète si le test échoue
        # catch_exceptions=False
    )
    print(result.output)

    # 1. Vérification du code de sortie
    assert result.exit_code == 0, f"La CLI a échoué : {result.output}"

    # 2. Vérification de la sortie (affichage du jeton)
    # La vue auth.py affiche : console.print("Your token is : ") et console.print(token)
    assert 'Your token is :' in result.output
